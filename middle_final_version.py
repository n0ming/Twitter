...
header = ['AttachCheck','Profile','Url','Date','Content','ImageUrl','AttachTweet','AttachUrl']
sub_link = False
Index = 'X'
...
def save_csv(browser_url,browser):
    ...
    global Index 
    row_data = []
    Profile = Date = Origin_Content = ImageUrl = AttachTweet = AttachUrl = ''
    sub_link =False
    Index = 'X'
    while True:
        ...
        Origin_Content= maintext(browser)  #Twitter Content 함수
...
def all_following_tweet_url(browser):
    tweet_urls =[]
    desired_date = datetime.strptime("2023-08-08T03:00:00.000Z", "%Y-%m-%dT%H:%M:%S.%fZ")
